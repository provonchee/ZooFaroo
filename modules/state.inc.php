...
var myImagePR = new Image;
	...
	fetchCityObject.fetchCityArray('<? echo $state; ?>', false);
	...
	function displayCities(citiesArray){
		...
		chosenOfferNeed = 'X10';
		   ...
		   $('#state-image').html('<img name="n20" src="images/states/'+citiesArray[0][0]+'.gif" border="0" id="n'+citiesArray[0][0]+'" usemap="#m_'+citiesArray[0][0]+'" alt="'+chosenState+'" /><map name="m_'+citiesArray[0][0]+'" id="m_'+citiesArray[0][0]+'"></map>');
		   ...
		   for(var i=0; i<(citiesArray.length-1); i++){
		    ...
		    $('.stateBase #state-menu').css('margin-left', stateMenuMarginLeft+'px');
