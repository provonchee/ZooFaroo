...
$("input[name='search_goodService']").change(function(){
											   ...
											   chosenGS[1] = $("input[name='search_goodService']:checked").attr('aux');
...
$('#search-categoryList #changeSelection').unbind('click').click(function(){
											$("#search-categoryList").html('<input type="radio" name="search_goodService" aux="g" value="Goods"> Goods &nbsp;&nbsp;<input type="radio" name="search_goodService" aux="s" value="Services">Services');
											catActivate();
										});
}
...
function displayCategories(parsedCategories){
	...
	$("#search_categories").prepend('<option value="please choose...">please choose...</option>');
			for(i=1; i<parsedCategories.length; i++){
										...
										$("#search_categories").append('<option value="'+drpDwnCategoryID+'" aux="'+drpDwnCategoryAlt+'">'+drpDwnCategory+'</option>');
										}
																	 }
																	 
							$("#search_categories").unbind('change').change(function(e){
									drpDwnCategory = $("#search_categories option:selected").attr('aux').replace(/_/g, ' ');
									drpDwnCategoryID = $("#search_categories option:selected").val();
									drpDwnCategoryAlt = $("#search_categories option:selected").attr('aux');
